...
class StrandsLeaderboard extends StatefulWidget {
  final AppTheme theme;
  final VoidCallback onBack;
  const StrandsLeaderboard({super.key, required this.theme, required this.onBack});

  @override
  State<StrandsLeaderboard> createState() => _StrandsLeaderboardState();
}

class _StrandsLeaderboardState extends State<StrandsLeaderboard> {
  ...
  Future<void> _load() async {
    try {
      final data = await StrandsApi.getLeaderboard();
  ...
  Widget _buildTable(String title, List<Map<String, dynamic>> entries, String valueKey, String suffix) {
    return Column(
          ...
          return Container(
            ...
            child: Row(
              children: [
                ...
                Text('${entry[valueKey]} $suffix', style: TextStyle(color: isMe ? widget.theme.correct : Colors.grey, fontSize: 14)),
              ],
            ),
          );
        }),
                    ...
                    child: LayoutBuilder(builder: (context, constraints) {
                      if (constraints.maxWidth > 500) {
                        return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Expanded(child: _buildTable("Today", _daily, 'hintsUsed', 'hints')),
                          const SizedBox(width: 24),
                          Expanded(child: _buildTable('Monthly Avg', _monthly, 'avgHints', 'avg')),
                        ]);
                      }
                      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        _buildTable("Today", _daily, 'hintsUsed', 'hints'),
                        const SizedBox(height: 24),
                        _buildTable('Monthly Avg', _monthly, 'avgHints', 'avg'),
                      ]);
                    }),
                  ),
                ),
